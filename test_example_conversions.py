...
def _test_pandoc(result_file, command, *, expected_file_size, size_deviation=0):
    try:
        ...
        if size_deviation > 0:
            min_size = expected_file_size - size_deviation
            max_size = expected_file_size + size_deviation
...
def test_html_generation():
    ...
    _test_pandoc('README.html', to_html, expected_file_size=1457, size_deviation=200)


def test_pdf_generation():
    to_pdf = 'docker run --rm -v "$(pwd):/pandoc" geometalab/pandoc pandoc -o {} README.md'
    _test_pandoc('README.pdf', to_pdf, expected_file_size=106515, size_deviation=200)
...
def test_pdf_generation_using_html5_and_wkhtml():
    ...
    _test_pandoc('README.pdf', to_pdf, expected_file_size=22349, size_deviation=200)
